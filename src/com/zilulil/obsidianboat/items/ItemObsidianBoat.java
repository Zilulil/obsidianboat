package com.zilulil.obsidianboat.items;
...
{
	private static final String __OBFID = "CL_15000001";
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    EntityObsidianBoat entityobsidianboat = new EntityObsidianBoat(par2World, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    entityobsidianboat.rotationYaw = (float)(((MathHelper.floor_double((double)(par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!par2World.getCollidingBoundingBoxes(entityobsidianboat, entityobsidianboat.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return par1ItemStack;
                    }
                    ...
                    {
                        par2World.spawnEntityInWorld(entityobsidianboat);
                    }
